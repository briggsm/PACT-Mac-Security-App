...
if [ "$1" == "-w" ]; then
    ...
    sudo -u $SUDO_USER defaults write /users/$SUDO_USER/Library/Preferences/ByHost/com.apple.screensaver askForPassword -int 1

    # Note: DOES NOT NEED sudo
    sudo -u $SUDO_USER defaults write /users/$SUDO_USER/Library/Preferences/ByHost/com.apple.screensaver askForPasswordDelay -int 0
